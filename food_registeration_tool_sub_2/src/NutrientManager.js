...
const NutrientManager = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Failed to add nutrient: ${errorMessage}`);
            }
            ...
            setNewNutrient({ name: '', unit: '', number: '', rank: '' }); // Reset the form
        } catch (error) {
    ...
    return (
        <div>
            ...
            {/* Table to display list of nutrients */}
            <table>
                <thead>
                    <tr>
                        <th>Name</th>
            ...
            {/* Button to load more nutrients */}
            <button onClick={fetchNutrients}>Load More Nutrients</button>
        </div>
    );
};
